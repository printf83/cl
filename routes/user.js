							...
							bcrypt.compare(password, data.password, function (err, res) {
								if (!err) {
									if (res === true) {
										...
										res({ username: username, id: data.id, result: true });
									} else {
										//send {id:"Not found"} if not found
										res({ username: username, result: "Not found" });
									}
								}
							});
						} else {
							//send {username:"Not found"} if not found
							res({ username: username, result: "Not found" });
						}
					})
					.catch((err) => {
						//send {username: username, result: "Error Message"} if something wrong
						res({ username: username, result: err.message });
					});
			} else {
			...
			try {
				...
				$.db
					...
					.then((data) => {
						...
						//remove password
						t[0].data = t[0].data.map(({ password, ...i }) => i);
			...
			if (i && i.length > 0) {
				if (i.length === 1) {
					...
					Promise.all(p)
						.then((result) => {
							//remove password
							result = result.map(({ password, ...i }) => i);
