	...
	set data(opt) {
		if (opt) {
				...
				let btn = opt.button.map((j, ix) => {
					if (j.hasOwnProperty("cl")) {
						...
						return new button({
							...
							label: i.label,
							icon: i.icon,
							class: core.mergeClass(i.class, "btn-modal"),
							color: ix === 0 ? (i.color ? i.color : defButtonColor) : i.color ? i.color : "transparent",
							textBgColor:
								ix === 0 ? (i.color ? i.color : defButtonColor) : i.color ? i.color : opt.color,
			...
			if (ctlButton || ctlControl) {
				if (opt.centerbutton) {
					ctlFooter = new div({
						borderNone: !opt.divider ? "top" : null,
						class: ["modal-footer"],
						elem: new div({
							gap: 2,
							padding: 0,
			...
			super.data = {
				id: opt.id,
				name: opt.name,
				class: core.mergeClass(opt.class, [
					"modal",
					opt.animate && !opt.debug && "fade",
					!opt.debug ? "show" : "cl-debug",
				]),
				style: core.mergeStyle(opt.style, opt.debug ? { display: "block", position: "static" } : null),
				tabindex: -1,
				...
				"show.bs.modal": opt.show,
				"shown.bs.modal": opt.shown,
				"hide.bs.modal": opt.hide,
				"hidden.bs.modal": opt.hidden,
				"hidePrevented.bs.modal": opt.hideprevented,
